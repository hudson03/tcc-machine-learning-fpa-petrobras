...
def parse_bcb_monthly_pages():
    observations = {}
    for filename in ("bcb_sgs3698_page1_rows.json", "bcb_sgs3698_page2_rows.json"):
        rows = json.loads((EXT / filename).read_text(encoding="utf-8"))
        for row in rows:
            flat = " ".join(clean_tokens(row))
            match = re.search(r"\b(jan|fev|mar|abr|mai|jun|jul|ago|set|out|nov|dez)/(20\d{2})\s+([0-9]+,[0-9]+)", flat, re.I)
            if match:
                month = MONTHS_PT[match.group(1).lower()]
                year = int(match.group(2))
                observations[(year, month)] = float(match.group(3).replace(",", "."))
    return observations


def parse_bcb_daily_fallback():
    monthly = defaultdict(list)
    for filename in ("bcb_usdbrl_2006_2015.json", "bcb_usdbrl_2016_2025.json"):
        data = json.loads((EXT / filename).read_text(encoding="utf-8"))
        for item in data:
            dt = datetime.strptime(item["data"], "%d/%m/%Y")
            monthly[(dt.year, dt.month)].append(float(item["valor"]))
    return {key: float(np.mean(values)) for key, values in monthly.items()}
...
result = {
    "metadata": {
        ...
        "driver_sources": {
            ...
            "USD_BRL": "Banco Central do Brasil, SGS 3698, média mensal de período (venda)",
        },
...
(OUTPUT_DIR / "driver_model_results.json").write_text(json.dumps(result, ensure_ascii=False), encoding="utf-8")
print(json.dumps({
